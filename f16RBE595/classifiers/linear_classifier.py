...
class LinearClassifier:
  ...
  def train(self, X, y, learning_rate=1e-3, reg=1e-5, num_iters=100,
    ...
    for it in xrange(num_iters):
      ...
      #########################################################################
      pass
      batch_data_indices = np.random.choice(num_train, batch_size)
  ...
  def loss(self, X_batch, y_batch, reg):
    ...
    delta = 1
    loss_image_arr = np.empty([num_samples, num_classes])
    ...
    for i in range(num_samples):
      ...
      for j in range(num_classes):
        if (j!=y_batch[i]):
          loss_image_arr[i, j] = max(0, score_matrix[j, i] - score_matrix[y_batch[i], i] + delta)
